# Based on https://github.com/locuslab/deq/blob/1fb7059d6d89bb26d16da80ab9489dcc73fc5472/lib/solvers.py
...
def broyden(f, x0, threshold, eps=1e-3, stop_mode="rel", ls=False, name="unknown"):
    try:
        ...
        x0_ = x0.unsqueeze(-1)
        f_ = lambda x_: f(x_.squeeze(-1)).unsqueeze(-1)
        bsz, total_hsize, seq_len = x0_.size()
    ...
    Us = torch.zeros(bsz, total_hsize, seq_len, threshold).to(dev).type(x0.dtype)     # One can also use an L-BFGS scheme to further reduce memory
    VTs = torch.zeros(bsz, threshold, total_hsize, seq_len).to(dev).type(x0.dtype)
    ...
    return {"result": lowest_xest.view_as(x0),
